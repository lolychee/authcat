...
module Authcat
  class IdentityProviderGenerator < ActiveRecord::Generators::Base
    ...
    def generate_model
      invoke "active_record:model",
             [model_name, "#{singular_name}:belongs_to", "provider:string", "token:string", "metadata:string",
              *attributes], **options
    end

    def inject_marco_content
      inject_into_file "app/models/#{model_singular_name}.rb", "  include Authcat::IdentityProvider::Record \n\n",
                       after: /class #{model_name}.*\n/
      inject_into_file "app/models/#{singular_name}.rb", "  has_many_identity_providers\n",
                       after: /class #{name}.*\n/
    end
