  ...
  module Password
    module Validators
      class ChallengeValidator < ActiveModel::EachValidator
        ...
        unless (challenge = record.public_send("#{attribute}_challenge")).nil?
          unless challenge_value_equal?(record, attribute, value, challenge)
            human_attribute_name = record.class.human_attribute_name(attribute)
            record.errors.add(:"#{attribute}_challenge", :challenge, **options.except(:case_sensitive).merge!(attribute: human_attribute_name))
          end
        end
      end
        ...
        private
          ...
          def challenge_value_equal?(record, attribute, value, challenge)
