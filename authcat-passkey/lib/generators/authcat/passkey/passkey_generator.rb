...
module Authcat
  class PasskeyGenerator < ActiveRecord::Generators::Base
    ...
    def generate_model
      invoke "active_record:model",
             [model_name, "#{singular_name}:belongs_to", "webauthn_id:string", "title:string", "public_key:string", "sign_count:integer", *attributes], **options
    end

    def inject_marco_content
      inject_into_file "app/models/#{model_singular_name}.rb", "  include Authcat::Passkey::Record \n  self.primary_key = :webauthn_id\m\n",
                       after: /class #{model_name}.*\n/
      inject_into_file "app/models/#{singular_name}.rb", "  has_many_passkeys\n",
                       after: /class #{name}.*\n/
    end
